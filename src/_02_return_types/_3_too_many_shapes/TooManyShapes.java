...
public class TooManyShapes {
	...
	public static void main(String[] args) {
		...
		String sideInput = JOptionPane.showInputDialog("How many sides do you want on your shape?");
		int sides = Integer.parseInt(sideInput);
		//2. Ask the user how many shapes they want
		String shapeNumInput = JOptionPane.showInputDialog("How many shapes do you want?");
		int shapeNum = Integer.parseInt(shapeNumInput);
		//3. Call canMakeShape() and save what is returned into a variable
		boolean ableToDraw = canMakeShape(sides);
		//4. If the shape CAN be drawn
		if(ableToDraw) {
			int angle = calculateTurnAngle(sides);
			drawPolygons(sides, shapeNum, angle);
		}else {
